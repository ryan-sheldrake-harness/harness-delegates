...
RUN mkdir -m 777 -p client-tools/kubectl/v1.28.7 \
  ...
  && mkdir -m 777 -p client-tools/gcloud \
  && cd /opt/harness-delegate/client-tools/gcloud && curl -O https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-476.0.0-linux-x86_64.tar.gz \
  && tar -xvf google-cloud-cli-476.0.0-linux-x86_64.tar.gz \
  && rm -rf google-cloud-cli-476.0.0-linux-x86_64.tar.gz \
...
ENV PATH=/opt/harness-delegate/client-tools/gcloud/google-cloud-sdk/bin/:$PATH
